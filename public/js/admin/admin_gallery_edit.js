window.onload = async () => {
    ...
    try {
        const response = await fetch(`http://angelespeluditos.cl:3000/api/post/find/${postId}`, {
            method: 'GET',
            mode: 'cors',
            headers: {
                'Authorization': token
...
document.getElementById('editarPostForm').addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch(`http://angelespeluditos.cl:3000/api/post/update/${postId}`, options);
