const cargarBanners = async () => {
  ...
  try {
      const response = await fetch('https://angelespeluditos.cl/api/api/banner/listDisabled', {
          method: 'GET',
          mode: 'cors',
          headers: {
              'Authorization': token
      ...
      let elementosHTML = banners.map(banner => `
          <div class="card d-flex flex-column gap-1 justify-content-center card-responsive">
              <img src="https://angelespeluditos.cl/api/api/banner/showimage/${banner._id}" style="width: 100%; aspect-ratio: 3/2; object-fit: cover; border-radius: 15px;">
