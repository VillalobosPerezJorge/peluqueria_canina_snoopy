...
const cargarPosts = async (windowPage) => {
  ...
  try {
      const response = await fetch(`http://localhost:3000/api/post/listPosts/${windowPage}`);
    ...
    setTimeout(() => {
        ...
        btns_seleccionar.forEach(btn => {
          ...
          btn.addEventListener('click', async e => {
            ...
            try {
              const response = await fetch(`http://localhost:3000/api/post/changeSelected/${postId}`,
                {
                  method: 'PATCH',
                  mode: 'cors',
                  headers: {
