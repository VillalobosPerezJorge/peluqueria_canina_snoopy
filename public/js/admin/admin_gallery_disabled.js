...
const cargarPosts = async (windowPage) => {
  ...
  try {
      const response = await fetch(`http://localhost:3000/api/post/listDisabled/${windowPage}`, {
        method: 'GET',
        mode: 'cors',
        headers: {
          'Authorization': token
    ...
    <div class="card d-flex flex-column gap-1 justify-content-center card-responsive">
      <img-comparison-slider style="width: 95%; max-height: 360px; margin-top: 10px; border-radius: 15px;">
        <img slot="first" src="http://localhost:3000/api/post/showImage/${post._id}/1" style="width: 100%; aspect-ratio: 3/2; object-fit: cover" />
        <img slot="second" src="http://localhost:3000/api/post/showImage/${post._id}/2" style="width: 100%; aspect-ratio: 3/2; object-fit: cover" />
    ...
    for(let i = dataPage - 1; i <= dataPage + 1; i++){
      if(i !== 0 && i <= totalPages){
        pagesArray.push(`<li class="page-item"><a class="page-link ${dataPage == i ? 'active' : ''}" href="/admin/vista-banner-inactivos/${i}">${i}</a></li>`);
      } else if(i == 1) {
        pagesArray = [`<li class="page-item"><a class="page-link ${dataPage == 1 ? 'active' : ''}" href="/admin/vista-banner-inactivos/1">1</a></li>`]
      }
    }

    pagesArray.unshift(`
  <li class="page-item">
    <a class="page-link ${data.hasPrevPage ? '' : 'disabled'}"  href="/admin/vista-banner-inactivos/1" aria-label="Previous">
      <span aria-hidden="true">Inicio</span>
    </a>
  </li>
  <li class="page-item">
    <a class="page-link ${data.hasPrevPage ? '' : 'disabled'}" href="/admin/vista-banner-inactivos/${dataPage - 1}" aria-label="Previous">
  ...
  pagesArray.push(`
  <li class="page-item">
    <a class="page-link ${data.hasNextPage ? '' : 'disabled'}" href="/admin/vista-banner-inactivos/${dataPage + 1}" aria-label="Next">
      <span aria-hidden="true">&raquo;</span>
    </a>
  </li>
  <li class="page-item">
    <a class="page-link ${data.hasNextPage ? '' : 'disabled'}" href="/admin/vista-banner-inactivos/${totalPages}" aria-label="Next">
    ...
    setTimeout(() => {
        ...
        btns_habilitar.forEach(btn => {
          ...
          btn.addEventListener('click', async e => {
            ...
            try {
              const response = await fetch(`http://localhost:3000/api/post/changeStatus/${postId}`,
                {
                  method: 'Post',
                  mode: 'cors',
                  headers: {
