...
const cargarBannersActivos = async (windowPage) => {
    ...
    try {
        const response = await fetch(`http://localhost:3000/api/banner/list/${windowPage}`, {
            method: 'GET',
            mode: 'cors',
            headers: {
                'Authorization': token
        ...
        const elementosHTML = banners.map(banner => `<!-- Galería-->
            <div class="card d-flex flex-column gap-1 justify-content-center card-responsive">
                <img src="http://localhost:3000/api/banner/showImage/${banner._id}" style="width: 95%; max-height: 360px; margin-top: 10px; border-radius: 15px; object-fit: cover;" />
